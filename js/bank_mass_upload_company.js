...
handleMassUpload = (e) => {
    ...
    result.forEach((company, index) => {
        if (!(company.name && company.email && company.companyId && validateEmail(company.email))) {
            if(validated) {
                validated = false;
    ...
    if(validated) {
        ...
        console.log(result);
        axios.post(baseURL + '/bank/companies/mass', {companies: result})
        .then(response => {
...
document.getElementById('fileInput').addEventListener('click', (e) => {e.target.value = null;});
document.getElementById('massUpload').addEventListener('click', handleMassUpload);