...
const loadCompany = () => {
  ...
  axios
    .get(baseURL + "/bank/companies/" + companyId + "/settings")
    .then((responseSettings) => {
      const settings = responseSettings.data;
      $("#companyUpdatePeriod").val(settings.updatePeriod);
      $("#companyAllowedToChangeConnection").val(
        settings.allowedToChangeConnection.toString()
      );
      ...
      companyForm.addEventListener("submit", (e) => {
        ...
        axios
          .put(
            baseURL + "/bank/companies/" + companyId + "/settings",
            companyBody
          )
          .then((response) => {
